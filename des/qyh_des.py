...
def createKeys(inkeys):
    ...
    for i in range(16):
        ...
        if (i==0 or i==1 or i==8 or i==15):
            ...
            moveStep=2
        keyc=key0[:28]
        keyc=keyc[moveStep:]+keyc[:moveStep]
        keyd=key0[28:]
        keyd=keyd[moveStep:]+keyd[:moveStep]
        key0=keyc+keyd
        for k in range(48):
...
def des_e(text,key):
    ...
    text = text+(length%4)*" "
    #print(keyResult)
    tempText=[0 for i in range(64)]#临时盛放IP逆置换前，将L部分和R部分合并成64位的结果
...
#-----------IP逆置换--------
    ...
    finalout=str(finalTextOfBit).strip('[]').replace(',','').replace(' ','')
    return finalout
#64位分组解密
def des_d(bitText,key):
    keyResult=createKeys(key)
    finalTextOfBit=[0 for i in range(64)]
    finalTextOfUnicode=[0 for i in range(4)]
    tempText=[0 for i in range(64)]#临时盛放IP逆置换前，将L部分和R部分合并成64位的结果
...
#加密前的分组
def _des_e(mtext,key):
    length=len(mtext)
    mtext=mtext+(length%4)*" "
    e_result=''
    text=[]
    result=[]
    for i in range(0,len(mtext),4):
        text=[]
        result=[]
        text=mtext[i:i+4]
        result=des_e(text,key)
        e_result=e_result+(result)
    return e_result
#解密前的分组
def _des_d(text,key):
    bitText=[]
    for k in range(len(text)):
        bitText.extend(text[k])
        _temp=bitText[k]
        _temp=int(_temp)
        bitText[k]=_temp
    text=[]
    result=[]
    d_result=''
    for i in range(0,len(bitText),64):
        text=bitText[i:i+64]
        result=des_d(text,key)
        d_result=d_result+str(result)
    return d_result
...
if __name__ == '__main__' :
    ...
    while i==0:
        ...
        if optionflag == 0:
            key=input('请输入8位加密密钥:')
            while(len(key)!=8):
                key = input("wrong!请输入8位加密密钥: ")
            with open('M_0.txt') as mfile:
                mtext = mfile.read()
            e_result = _des_e(mtext,key)
            with open('C.txt','w') as cfile:
                cfile.write(str(e_result))
            os.system('pause')
        elif optionflag == 1:
            key=input('请输入8位解密密钥:')
            while(len(key)!=8):
                key = input("wrong!请输入8位解密密钥: ")
            with open('C.txt','r') as cfile:
                ctext = cfile.read()
            d_result = _des_d(ctext,key)
            with open('M_1.txt','a') as mfile:
                mfile.write('\n'+'\n解密得到的明文为：' +'\n'+ str(d_result))
            print('\n解密得到的明文为：' +'\n'+ str(d_result))
            os.system('pause')
            os.system('cls')
